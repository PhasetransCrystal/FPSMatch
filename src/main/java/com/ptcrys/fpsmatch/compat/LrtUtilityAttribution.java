...
public final class LrtUtilityAttribution {
    ...
    public static void ignited(Entity target, Entity source, LivingEntity owner, int seconds) {
        if (!(target instanceof ServerPlayer victim) || !(owner instanceof ServerPlayer thrower)
                || seconds <= 0 || !victim.isOnFire()) return;
    ...
    public static DamageSource resolve(ServerPlayer victim, DamageSource source) {
        ...
        if (!victim.isOnFire() || victim.level().getGameTime() > fire.expires
                || FPSMCore.getInstance().getMapByPlayer(victim).orElse(null) != fire.map
                || fire.map.getMapTeams().getPlayerData(victim).orElse(null) != fire.data
                || fire.data.getDeaths() != fire.deaths || fire.data.getCombatRevision() != fire.revision
                || !fire.map.isStart() || !fire.map.checkGameHasPlayer(fire.owner)) {
            FIRES.remove(victim);
            return source;
        }
    ...
    public static LivingEntity owner(Entity source) {
        ...
        return source instanceof TraceableEntity traceable && traceable.getOwner() instanceof LivingEntity living
                ? living : null;
    }
