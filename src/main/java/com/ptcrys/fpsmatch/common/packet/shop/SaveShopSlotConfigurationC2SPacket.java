...
public record SaveShopSlotConfigurationC2SPacket(int containerId, int ammo, int price, int group, List<String> modules) {
    public SaveShopSlotConfigurationC2SPacket { modules = List.copyOf(modules); }
    public static void encode(SaveShopSlotConfigurationC2SPacket packet, FriendlyByteBuf buf) {
        ...
        buf.writeInt(packet.ammo); buf.writeInt(packet.price); buf.writeInt(packet.group);
    ...
    public void handle(Supplier<NetworkEvent.Context> context) {
        context.get().enqueueWork(() -> {
            ...
            var result = player.containerMenu instanceof EditShopSlotMenu menu && menu.containerId == containerId
                    ? menu.trySaveData(player, ammo, price, group, modules) : EditShopSlotMenu.SaveResult.INVALID_MENU;
