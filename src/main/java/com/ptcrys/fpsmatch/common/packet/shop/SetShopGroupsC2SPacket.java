...
public record SetShopGroupsC2SPacket(int containerId, long requestId, int groupId, int[] indices) {
    public SetShopGroupsC2SPacket { indices = indices.clone(); }
    public static void encode(SetShopGroupsC2SPacket packet, FriendlyByteBuf buf) {
        ...
        buf.writeInt(packet.groupId); buf.writeVarIntArray(packet.indices);
    }
    ...
    public void handle(Supplier<NetworkEvent.Context> context) {
        context.get().enqueueWork(() -> {
            ...
            var result = player.containerMenu instanceof EditorShopContainer menu && menu.containerId == containerId
                    ? menu.trySetGroups(player, indices, groupId) : EditShopSlotMenu.SaveResult.INVALID_MENU;
