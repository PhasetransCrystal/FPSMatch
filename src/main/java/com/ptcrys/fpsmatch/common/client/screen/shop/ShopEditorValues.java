...
public final class ShopEditorValues {
    ...
    public static boolean validGroup(int group) { return group >= -1 && group <= 999_999; }
    ...
    public static boolean validModules(List<String> names, Collection<String> registered) {
        ...
        for (String name : names) if (name == null || name.isBlank() || name.length() > MAX_MODULE_NAME
                || !registered.contains(name) || !unique.add(name)) return false;
        return true;
    }
}
