...
public abstract class ModernMenuScreen<T extends AbstractContainerMenu> extends ModernScreen implements MenuAccess<T> {
    ...
    protected ModernMenuScreen(T menu, Component title) { super(title, null); this.menu = menu; }
    @Override public T getMenu() { return menu; }
    @Override public boolean isMenuScreen() { return true; }
    ...
    @Override public void tick() {
        Minecraft mc = Minecraft.getInstance();
        if (mc.player == null || mc.player.containerMenu != menu) { mc.setScreen(null); return; }
        refresh();
    }
