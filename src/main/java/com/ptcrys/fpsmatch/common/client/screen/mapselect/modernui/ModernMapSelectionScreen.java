...
public final class ModernMapSelectionScreen extends ModernScreen {
    ...
    @Override public void tick() {
        if (opening && ++requestTicks >= 200) {
            opening = false; status = tr("gui.fpsm.shop_editor.open.timeout"); refresh();
        }
    }
    @Override protected List<Node> content() {
        int margin = Math.min(16, Math.max(8, width / 32)), w = Math.max(1, width - margin * 2);
        var layout = com.ptcrys.fpsmatch.common.client.screen.mapselect.modernui.MapSelectionLayoutModel.responsive(w, Math.max(1,height-16));
        ...
        nodes.add(text("rooms.heading",tr("gui.fpsm.map_select.rooms.title")).at(margin,top,w,16)); top+=20;
        boolean wrap=w<250;
        ...
        List<Node> rooms=new ArrayList<>(); String search=query.toLowerCase(Locale.ROOT);
        for(MapRoomSummary room:snapshot.maps()) {
            ...
            if(state.equals("waiting")&&room.started() || state.equals("running")&&!room.started()
                    || state.equals("open")&&(room.full()||room.started()&&!room.allowJoinInProgress()))continue;
    ...
    @Override public void onClose() { FPSMatch.sendToServer(new CloseMapViewC2SPacket()); super.onClose(); }
}
