...
public final class FPSMTeamActionScreen extends ModernScreen implements FPSMMapDetailChildScreen {
    ...
    public FPSMTeamActionScreen(MapRoomDetail detail, Screen parent, MapRoomPlayerInfo player) { this(detail, parent, player, false); }
    ...
    private FPSMTeamActionScreen(MapRoomDetail detail, Screen parent, MapRoomPlayerInfo player, boolean kick) {
        ...
        this.detail = detail; this.player = player.uuid(); this.kick = kick;
    }
    private boolean permitted() {
        return detail.summary().currentPlayerOp() || !kick && Minecraft.getInstance().player != null
    ...
    @Override public void applyDetail(MapRoomDetail next) {
        ...
        detail = next;
        if (!permitted() || detail.players().stream().noneMatch(p -> p.uuid().equals(player))) { onClose(); return; }
    ...
    @Override protected List<Node> content() {
        ...
        else for (var team : detail.teams()) body.add(button(team.name(), (team.name().equals(selected) ? "● " : "")
                + team.name() + "  " + team.currentPlayers() + "/" + (team.playerLimit() < 0 ? "∞" : team.playerLimit()),
    ...
    private void submit() {
        if (!permitted() || (kick ? !TeamActionModel.canKick(detail, player)
                : selected == null || !TeamActionModel.availableTargetTeams(detail, player).contains(selected))) return;
