...
public final class ModernMapRoomScreen extends ModernScreen implements FPSMMapDetailChildScreen {
    ...
    public boolean accepts(MapRoomDetail incoming) {
        return incoming.summary().gameType().equals(detail.summary().gameType()) && incoming.summary().mapName().equals(detail.summary().mapName());
    }
    ...
    public void applyDetail(MapRoomDetail next) {
        ...
        if (pending.equals("settings")) {
            submitted.entrySet().removeIf(e -> next.settings().stream().anyMatch(s -> s.name().equals(e.getKey()) && equivalent(s, e.getValue())));
            drafts.entrySet().removeIf(e -> next.settings().stream().anyMatch(s -> s.name().equals(e.getKey()) && equivalent(s, e.getValue())));
            if (submitted.isEmpty()) {
                pending = "";
                status = tr("gui.fpsm.map_select.settings.save_success", savedCount);
            }
        }
    ...
    public void applySources(MapImportSourcesS2CPacket packet) {
        ...
        status = tr(sources.isEmpty() ? "gui.fpsm.map_import.empty" : "gui.fpsm.map_import.select");
        refresh();
    }
    ...
    public void applyToast(MapRoomToastS2CPacket packet) {
        ...
        if (generalError || pending.equals("settings") && packet.error() && key.contains(".setting.") || pending.equals("regions") && key.startsWith("gui.fpsm.map_regions.") || pending.equals("import") && key.startsWith("gui.fpsm.map_import.") || pending.equals("editor") && key.startsWith("gui.fpsm.shop_editor.open.")) {
    ...
    private boolean operator() {
        return detail.summary().currentPlayerOp();
    }
    ...
    public void onClose() {
        ...
        if (page == Page.SHOPS || page == Page.REGIONS || page == Page.IMPORT || page == Page.INVITE) {
            navigate(page == Page.INVITE ? Page.PLAYERS : Page.SETTINGS);
            return;
        }
        requestLeave(() -> {
    ...
    protected List<Node> content() {
        if (contextPlayer != null) {
            ...
            if (player == null) contextPlayer = null;
                    ...
                    button("move", tr("gui.fpsm.team_manage.context.switch"), (operator() || player.uuid().equals(selfId())) && !TeamActionModel.availableTargetTeams(detail, player.uuid()).isEmpty(), () -> {
                        contextPlayer = null;
        ...
        nodes.add(title("title", switch (page) {
            case REGIONS -> tr("gui.fpsm.map_regions.title");
            case IMPORT -> tr("gui.fpsm.map_import.title");
            case INVITE -> tr("gui.fpsm.map_select.invite.title");
            case SHOPS -> tr("gui.fpsm.map_shop.title");
            default -> detail.summary().displayName();
        }).at(margin, 8, w, 21));
        ...
        if (page == Page.REGIONS) {
            ...
            actions.add(regionActions.children().get(1));
            actions.add(regionActions.children().get(0));
            actions.add(regionActions.children().get(2));
        ...
        if (page == Page.SETTINGS) {
            ...
            for (Node n : childBody) {
                switch (n.key()) {
                    ...
                    case "pos1.field" -> {
                        editor.add(n.children().get(0).at(8, 38, 48, 16));
                        editor.add(n.children().get(1).at(62, 32, Math.max(32, ew - 154), 24));
                    }
                    case "pos2.field" -> {
                        editor.add(n.children().get(0).at(8, 67, 48, 16));
                        editor.add(n.children().get(1).at(62, 61, Math.max(32, ew - 154), 24));
                    }
            ...
            nodes.add(text("status", status.isBlank() ? tr(detail.editableShops().isEmpty() ? "gui.fpsm.map_shop.unsupported" : "gui.fpsm.map_shop.selection.ready") : status)
            ...
            List<Node> body = switch (page) {
                case DETAILS -> details();
                case PLAYERS -> players();
                case MORE -> management();
                case INVITE -> invitations();
                case SHOPS -> shops();
            ...
            return List.of(canvas("confirmation", List.of(canvas("dialog", List.of(
                    ...
                    row("actions", button("keep", tr("gui.fpsm.map_select.settings.keep_editing"), true, () -> discardDestination = null),
                            button("discard", tr("gui.fpsm.map_select.settings.discard"), true, () -> {
                                Runnable destination = discardDestination;
                                discardDestination = null;
                                drafts.clear();
                                regionDirty = false;
                                addingRegion = false;
                                loadRegion();
                                if (destination != null) destination.run();
                            })).at(8, dh - 32, dw - 16, 24)))
                    .surface().at(dx, dy, dw, dh))).fill());
        }
    ...
    private List<Node> details() {
        ...
        nodes.add(text("ready", tr("gui.fpsm.team_manage.ready_summary", ready.size(), detail.players().stream().filter(p -> !p.spectator()).count())));
        return nodes;
    }

    private List<Node> membership() {
        var room = detail.summary();
        boolean joined = room.currentPlayerJoined() || room.currentPlayerSpectating();
    ...
    private List<Node> players() {
        ...
        for (MapRoomTeamInfo team : detail.teams().stream().sorted(Comparator.comparing(MapRoomTeamInfo::spectator).thenComparing(MapRoomTeamInfo::name)).toList()) {
            ...
            rows.add(button("team", (collapsedTeams.contains(team.name()) ? "+ " : "− ") + team.name() + "  " + team.currentPlayers() + "/" + (team.playerLimit() < 0 ? "∞" : team.playerLimit()), true,
    ...
    private String readiness() {
        return tr("gui.fpsm.team_manage.ready_summary", ready.size(), detail.players().stream().filter(p -> !p.spectator()).count()) + (countdown > 0 ? " · " + tr("gui.fpsm.team_manage.countdown", countdown) : "");
    }
    ...
    private Node settingsPanel(int w, int h) {
        ...
        var groups = net.ptcrys.fpsmatch.common.client.screen.mapselect.modernui.MapSettingsGroupingModel.group(detail.settings(), selectedCategories);
        for (var group : groups) {
            ...
            for (var setting : group.settings()) {
                ...
                if (setting.type() == MapRoomSettingInfo.SettingType.BOOLEAN)
                    ...
                    controls.add(slider("slider", Math.max(0, Math.min(steps, progress)), steps, enabled, v -> {
                        ...
                        stage(setting.name(), next);
                    }).at(controlX, controlY, controlWidth - 52, 23));
            ...
            if (!entries.isEmpty()) {
                settings.add(text("category." + group.category(), tr(MapRoomSettingInfo.categoryTranslationKey(group.category()))).size(-1, 22));
                settings.addAll(entries);
            }
        }
        ...
        if (categoryPopup) {
            ...
            for (String c : net.ptcrys.fpsmatch.common.client.screen.mapselect.modernui.MapSettingsGroupingModel.categories(detail.settings()))
                categories.add(toggle("category." + c, tr(MapRoomSettingInfo.categoryTranslationKey(c)), selectedCategories.contains(c), true,
                        v -> {
                            if (v) selectedCategories.add(c);
    ...
    private void stage(String name, String value) {
        ...
        clearConfirmation = false;
        if (value.equals(current.value())) drafts.remove(name);
        else drafts.put(name, value);
    }

    private boolean validDrafts() {
        return drafts.entrySet().stream().allMatch(e -> detail.settings().stream().anyMatch(s -> s.name().equals(e.getKey()) && s.editable() && normalize(s, e.getValue()).isPresent()));
    }
    ...
    private static boolean equivalent(MapRoomSettingInfo s, String reference, String value) {
        ...
        try {
            if (s.type() == MapRoomSettingInfo.SettingType.INTEGER || s.type() == MapRoomSettingInfo.SettingType.DECIMAL)
                return new BigDecimal(reference.trim()).compareTo(new BigDecimal(value.trim())) == 0 || s.type() == MapRoomSettingInfo.SettingType.DECIMAL && Float.toString(Float.parseFloat(value)).equals(reference);
    ...
    private BlockPos currentPosition() {
        return Minecraft.getInstance().player == null ? BlockPos.ZERO : Minecraft.getInstance().player.blockPosition();
    }

    private void loadRegion() {
        AreaData area = regionIndex < 0 || regionIndex >= detail.bombAreas().size() ? detail.mapArea() : detail.bombAreas().get(regionIndex);
        pos1 = format(area.pos1());
        pos2 = format(area.pos2());
    }
    ...
    private List<Node> regions() {
        ...
        nodes.add(input("pos1", tr("gui.fpsm.map_regions.pos1"), pos1, editable, v -> {
            pos1 = v;
            regionDirty = true;
            deleteConfirmation = false;
        }));
        nodes.add(button("use1", tr("gui.fpsm.map_regions.use_position"), editable, () -> {
            pos1 = format(currentPosition());
            regionDirty = true;
        }));
        nodes.add(input("pos2", tr("gui.fpsm.map_regions.pos2"), pos2, editable, v -> {
            pos2 = v;
            regionDirty = true;
            deleteConfirmation = false;
        }));
        nodes.add(button("use2", tr("gui.fpsm.map_regions.use_position"), editable, () -> {
            pos2 = format(currentPosition());
            regionDirty = true;
        }));
        nodes.add(row("actions", button("save", tr("gui.fpsm.map_regions.apply"), editable, this::saveRegion),
                button("add", tr("gui.fpsm.map_regions.add"), editable && detail.demolitionRegionsSupported(), () -> requestLeave(() -> {
                    addingRegion = true;
                    regionIndex = -1;
                    pos1 = pos2 = format(currentPosition());
                    regionDirty = true;
                    refresh();
                })),
                button("delete", tr(deleteConfirmation ? "gui.fpsm.map_regions.delete_confirm" : "gui.fpsm.map_regions.delete"), editable && !addingRegion && regionIndex >= 0, () -> {
                    if (!deleteConfirmation) {
                        ...
                        status = tr("gui.fpsm.map_regions.delete_warning", regionIndex + 1);
    ...
    private void saveRegion() {
        ...
        if (a.isEmpty() || b.isEmpty()) {
            status = tr("gui.fpsm.map_regions.action.invalid");
            return;
        }
        regionAction(addingRegion ? MapRegionActionC2SPacket.Action.ADD_BOMB : regionIndex < 0 ? MapRegionActionC2SPacket.Action.SET_MAP : MapRegionActionC2SPacket.Action.UPDATE_BOMB, a.get(), b.get());
    }
    ...
    static Optional<BlockPos> parsePosition(String value) {
        ...
        try {
            return Optional.of(new BlockPos(Integer.parseInt(xyz[0]), Integer.parseInt(xyz[1]), Integer.parseInt(xyz[2])));
    ...
    private List<Node> imports() {
        ...
        for (MapImportSourceInfo item : sources) nodes.add(button("source." + item.id(), (source != null && source.id().equals(item.id()) ? "● " : "") + item.archiveName() + " / " + item.mapName(), idle(), () -> {
            source = item;
            importSettings = item.hasSettings();
            importShop = item.hasShop();
            importKits = item.hasStartKits();
            importConfirmation = 0;
        }));
        nodes.add(toggle("settings", tr("gui.fpsm.map_import.settings"), importSettings, source != null && source.hasSettings() && idle(), v -> {
            importSettings = v;
            importConfirmation = 0;
        }));
        nodes.add(toggle("shop", tr("gui.fpsm.map_import.shop"), importShop, source != null && source.hasShop() && idle(), v -> {
            importShop = v;
            importConfirmation = 0;
        }));
        nodes.add(toggle("kits", tr("gui.fpsm.map_import.kits"), importKits, source != null && source.hasStartKits() && idle(), v -> {
            importKits = v;
            importConfirmation = 0;
        }));
        nodes.add(button("refresh", tr("gui.fpsm.map_select.refresh"), idle(), this::requestSources));
        nodes.add(button("import", tr(importConfirmation == 0 ? "gui.fpsm.map_import.action" : importConfirmation == 1 ? "gui.fpsm.map_import.confirm" : "gui.fpsm.map_import.confirm_final"), operator() && idle() && source != null && (importSettings || importShop || importKits), () -> {
            if (!idle() || source == null || !operator()) return;
            if (importConfirmation++ < 2) {
                status = tr(importConfirmation == 1 ? "gui.fpsm.map_import.warning" : "gui.fpsm.map_import.warning_final", source.archiveName(), source.mapName());
                return;
            }
            begin("import");
            FPSMatch.sendToServer(new ImportMapConfigC2SPacket(detail.summary().gameType(), detail.summary().mapName(), source.id(), importSettings && source.hasSettings(), importShop && source.hasShop(), importKits && source.hasStartKits()));
        }));
        return nodes;
    }
}
