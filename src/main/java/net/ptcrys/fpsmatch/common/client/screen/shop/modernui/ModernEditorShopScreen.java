...
public final class ModernEditorShopScreen extends ModernMenuScreen<EditorShopContainer> {
    ...
    protected List<Node> content() {
        var layout = net.ptcrys.fpsmatch.common.client.screen.shop.modernui.ShopEditorLayoutModel.responsive(Math.max(1, width - 16), Math.max(1, height - 16));
        ...
        for (var entry : menu.getTypes().entrySet()) {
            ...
            categories.add(button("type." + type, type, !opening, () -> {
                category = type;
        ...
        if (selected >= 0 && selected < menu.getAllSlots().size() && menu.getAllSlots().get(selected) != null) {
            var slot = menu.getAllSlots().get(selected);
            var stack = menu.slots.get(selected).getItem();
        ...
        nodes.add(place(canvas("header", List.of(title("title", getTitle().getString()).at(0, 0, width - 160, 20),
                ...
                button("batch", tr("gui.fpsm.shop_editor.batch.title"), !opening, () -> {
                    batchMode = true;
                    status = "";
                }).at(width - 154, 0, 126, 22))), layout.header()));
        Node categoryView = layout.compact() ? select("categories", category, categories.stream().map(n -> text(n.key().substring(5), n.text())).toList(), v -> {
            category = v;
            selected = menu.getTypes().get(v).startIndex();
        }) : scroll("categories", categories);
    ...
    private List<Node> batchContent() {
        ...
        nodes.add(button("done", tr("gui.fpsm.shop_editor.batch.done"), !savingGroups, () -> {
            batchMode = false;
            status = "";
        }).at(width - 132, 6, 124, 22));
    ...
    private void saveGroups() {
        ...
        savingGroups = true;
        ticks = 0;
        groupRequest = REQUESTS.incrementAndGet();
    ...
    private Node place(Node node, net.ptcrys.fpsmatch.common.client.screen.shop.modernui.ShopEditorLayoutModel.Rect rect) {
    ...
    public void onClose() {
        ...
        super.onClose();
        ShopEditorNavigation.returnFromEditor(menu.getGameType(), menu.getMapName(), menu.getTeamName());
    }
}
