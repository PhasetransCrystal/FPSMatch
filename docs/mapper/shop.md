...
分组 ID 把同一队商店中的相关槽位连接起来。购买或退回某个槽位时，FPSMatch 会通知同组的**其他槽位**；监听模块挂在哪个槽位上，就决定哪个槽位响应别人的操作。相同数字本身不定义“护甲升级”等业务含义。

可选监听模块由 FPSMatch 和玩法模组注册。编辑器只接受服务器列出的模块；地图制作者不能在界面中创建新的 Java 模块。内置的 `returnGoods`、两种护甲模块和 `changeItem_...` 需要按正确方向挂载。

具体功能、图形界面步骤、命令写法和完整护甲示例见[商店监听模块](shop-listener-modules.md)。
...
下一步：[配置商店监听模块](shop-listener-modules.md)。
